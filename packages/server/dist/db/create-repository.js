...
export async function createRepository() {
    if (!env.DATABASE_URL) {
        console.warn("[db] No DATABASE_URL configured — using in-memory repository. Data is ephemeral and memory will grow unbounded.");
    ...
    try {
        ...
        console.warn("[db] Failed to connect to PostgreSQL, falling back to in-memory repository:", err);
        return { repo: new InMemoryRepository(), mode: "in-memory" };
    }
}
